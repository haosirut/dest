//! Heartbeat protocol: nodes exchange pings to maintain liveness tracking.
...
/// Heartbeat manager for tracking peer liveness.
pub struct HeartbeatManager {
    pub config: HeartbeatConfig,
    peers: HashMap<String, PeerStateInfo>,
    node_id: String,
...
impl HeartbeatManager {
    ...
    /// Process an incoming heartbeat message and return a PeerEvent.
    pub fn process_heartbeat(
        ...
        let peer = self
            ...
            .or_insert_with(|| PeerStateInfo::new(node_id.to_string()));
        ...
        let event = match peer.status {
            ...
            PeerState::Alive => PeerEvent::PeerAlive(node_id.to_string()),
        };

        let _ = timestamp; // Timestamp used for ordering in production
        debug!(
    ...
    /// Advance the heartbeat timer. Marks peers as suspect/dead based on elapsed time.
    ...
    pub fn get_dead_peers(&self) -> Vec<&PeerStateInfo> {
        self.peers
            .values()
            .filter(|p| p.status == PeerState::Dead)
            .collect()
    }

    /// Get alive peers count.
    pub fn alive_count(&self) -> usize {
        self.peers.values().filter(|p| p.status == PeerState::Alive).count()
    }
    ...
    /// Register a peer in the tracking map.
    pub fn register_peer(&mut self, peer_id: String) {
        self.peers
            .entry(peer_id.clone())
            .or_insert_with(|| PeerStateInfo::new(peer_id));
    }
}
...
mod tests {
    ...
    fn test_process_heartbeat_new_peer() {
        let mut manager = HeartbeatManager::new("node1".to_string(), HeartbeatConfig::default());
    ...
    fn test_peer_recovery() {
        ...
        assert_eq!(manager.peers.get("node2").unwrap().status, PeerState::Alive);
    }
    ...
    fn test_get_dead_peers() {
        ...
        manager.tick(); // -> suspect
        manager.tick(); // -> dead
        let dead = manager.get_dead_peers();
        assert_eq!(dead.len(), 1);
        assert_eq!(dead[0].peer_id, "node2");
    }

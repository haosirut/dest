...
async fn cmd_start(data_dir: &str, api_addr: &str) -> Result<()> {
    ...
    // Initialize P2P node
    let p2p_config = soty_p2p::P2pConfig::default();
    let _node = soty_p2p::P2pNode::new(p2p_config).await?;
    info!("P2P node started: {}", _node.peer_id_str());

    // Initialize ledger
    let ledger_path = PathBuf::from(&expanded).join("ledger").join("soty.db");
    let _ledger = soty_ledger::LedgerStore::open(&ledger_path)?;
    info!("Ledger opened: {}", ledger_path.display());

    // Initialize billing
    let _billing = soty_billing::BillingAccount::new(rust_decimal::Decimal::ZERO);
    info!("Billing initialized");

    // Initialize storage
    let shard_path = PathBuf::from(&expanded).join("shards");
    let _storage = soty_storage::shard_store::ShardStore::new(&shard_path)?;
...
fn cmd_stop() -> Result<()> {
    info!("Attempting to stop SOTY daemon...");
    // In production, this would send SIGTERM to the running process
    // or communicate via a UNIX socket / PID file
    warn!("Daemon stop: use 'systemctl stop sotyd' for systemd-managed nodes");
    Ok(())
}
...
async fn cmd_upload(file: &str, replication: u32) -> Result<()> {
    ...
    // Chunk the data
    let chunks = soty_core::chunking::chunk_data(&data);
    let chunk_ids = soty_core::chunking::generate_chunk_ids(&chunks);
...
async fn cmd_download(chunk_id: &str, _output: &str) -> Result<()> {
    let id = soty_core::ChunkId::from_hex(chunk_id)
...
async fn cmd_keys(action: KeyAction) -> Result<()> {
    match action {
        KeyAction::Generate => {
            let mnemonic = soty_core::bip39_recovery::generate_mnemonic(
                soty_core::MnemonicWordCount::Twelve,
            )?;
        ...
        KeyAction::Backup => {
            println!("Backup: Use 'sotyd keys generate' to create a new recovery mnemonic.");
        }
    }
    Ok(())
}
...
fn cmd_recover(mnemonic: &str, passphrase: &str) -> Result<()> {
    let parsed = soty_core::bip39_recovery::parse_mnemonic(mnemonic)?;
    let _key = soty_core::bip39_recovery::recover_key_from_mnemonic(&parsed, passphrase)?;
    ...
    println!("Run 'sotyd start' to resume operation with recovered key.");
    Ok(())
}

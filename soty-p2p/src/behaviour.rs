...
/// Composite behaviour for the SOTY P2P node.
...
impl Default for SOTYBehaviour {
    fn default() -> Self {
        Self::new()
    }
}
...
mod tests {
    ...
    fn test_behaviour_default() {
        let behaviour = SOTYBehaviour::new();
        assert!(!behaviour.is_fully_initialized());
    }

    #[test]
    fn test_behaviour_full_init() {
        let mut behaviour = SOTYBehaviour::new();
